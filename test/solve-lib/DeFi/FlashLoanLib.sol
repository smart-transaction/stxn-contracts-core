...
contract FlashLoanLib {
    ...
    function deployerLand(address pusher) public {
        ...
        flashLoan = new MockFlashLoan(dai, weth);
        dai.mint(address(flashLoan), 1009000000000000000000); // TODO cross check amount
        weth.mint(address(flashLoan), 1000000000000000000000); // TODO cross check amount
    }
    ...
    function solverLand(
        ...
        callObjs[0] = CallObject({
            ...
            callvalue: abi.encodeWithSignature(
                "approveTransfer(address,uint256,uint256)", address(daiWethPool), liquidity0, liquidity1  //TODO: call breaker to approve dai and weth (2) 
            )
        });
        ...
        callbreaker.executeAndVerify(abi.encode(callObjs), abi.encode(returnObjs), encodedData, hintdices); //TODO: executeAndVerify with 5 param 
    }
}
